...
5. **フラグAPI** (`/api/v1/flag/`) 🔒
   - SQLiteストレージによるリスクフラグ管理
   - サブジェクトIDによるフラグ取得（関連フラグ含む）
   - 複数サブジェクトへのフラグ一括作成
   - フラグIDによるフラグ削除
...
SQLiteストレージ（`data/flags.db`）を使用したリスクフラグ管理API。

#### サブジェクトIDでフラグ取得
サブジェクトに関連するすべてのフラグと、同じflag_idを共有する関連フラグを取得します。
```http
...
#### フラグ作成
同じflag_idで複数のサブジェクトIDに対するフラグレコードを作成します。
```http
POST /api/v1/flag
Authorization: Bearer <token>
Content-Type: application/json
